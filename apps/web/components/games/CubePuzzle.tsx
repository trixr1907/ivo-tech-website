...
function PuzzleCube({ position, color, targetColor, onClick, isCorrect, index }: CubeProps) {
...
export function CubePuzzle({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    const correctCount = cubeColors.filter((color, index) => color === targetPattern[index]).length;
    ...
    <div className='relative h-full w-full'>
      ...
      <div className='absolute left-4 top-4 z-10 rounded-lg bg-black/80 p-4 text-white backdrop-blur'>
        ...
        <div className='space-y-2 text-sm'>
          ...
          <div className='text-xs text-gray-300'>Click cubes to change colors!</div>
        </div>
      </div>
      ...
      <div className='absolute right-4 top-4 z-10 rounded-lg bg-black/80 p-4 text-white backdrop-blur'>
        ...
        <div className='grid grid-cols-3 gap-1'>
          {targetPattern.map((color, index) => (
            <div key={index} className='h-6 w-6 rounded border border-white/30' style={{ backgroundColor: color }} />
          ))}
        </div>
      </div>
