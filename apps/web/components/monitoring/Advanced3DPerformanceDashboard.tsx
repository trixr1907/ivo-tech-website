...
function PerformanceChart({ data, label, color, unit, threshold }: PerformanceChartProps) {
      ...
      <div className='relative h-20'>
        ...
        <svg className='absolute inset-0 h-full w-full' viewBox='0 0 100 100' preserveAspectRatio='none'>
          <polyline
            ...
            strokeWidth='2'
            points={data
              .map((value, index) => `${(index / (data.length - 1)) * 100},${100 - ((value - minValue) / range) * 100}`)
              .join(' ')}
          />
          <polyline
            fill={`${color}20`}
            ...
            points={[
              ...data.map(
                (value, index) => `${(index / (data.length - 1)) * 100},${100 - ((value - minValue) / range) * 100}`
              ),
              `100,100`,
              `0,100`,
            ].join(' ')}
...
function PerformanceGrade({ grade }: { grade: 'A' | 'B' | 'C' | 'D' | 'F' }) {
  ...
  return (
    <div className={`inline-flex items-center rounded-lg border px-4 py-2 ${gradeColors[grade]}`}>
...
function BottleneckIndicator({ type }: { type: 'cpu' | 'gpu' | 'memory' | 'audio' | 'none' }) {
  const bottleneckConfig = {
    ...
    memory: { icon: '💾', label: 'Memory Bottleneck', color: 'text-yellow-400' },
...
export function Advanced3DPerformanceDashboard() {
  const { metrics, /* updateMetrics, updateAudioMetrics, */ optimizePerformance, thresholds } =
  ...
  // Auto-Optimierung
  useEffect(() => {
    if ((isAutoOptimize && metrics.performanceGrade === 'D') || metrics.performanceGrade === 'F') {
  ...
  }, [metrics.performanceGrade, metrics.bottleneckType, isAutoOptimize, optimizePerformance]);
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-1 gap-6 lg:grid-cols-2'>
          ...
          <div className='space-y-4'>
            <div className='grid grid-cols-2 gap-4 text-sm'>
              <div>
                ...
                <span className='ml-2 font-mono text-white'>{metrics.activeObjects}</span>
              </div>
              <div>
                <span className='text-gray-400'>Visible Objects:</span>
                <span className='ml-2 font-mono text-green-400'>{metrics.visibleObjects}</span>
              </div>
              <div>
                <span className='text-gray-400'>Culled Objects:</span>
                <span className='ml-2 font-mono text-red-400'>{metrics.frustrumCulled}</span>
              </div>
              <div>
                <span className='text-gray-400'>Draw Calls:</span>
                <span className='ml-2 font-mono text-yellow-400'>{metrics.drawCalls}</span>
              </div>
              <div>
                <span className='text-gray-400'>Texture Memory:</span>
                <span className='ml-2 font-mono text-purple-400'>{metrics.textureMemory}MB</span>
              </div>
              <div>
                <span className='text-gray-400'>Geometry Memory:</span>
                <span className='ml-2 font-mono text-cyan-400'>{metrics.geometryMemory}MB</span>
              </div>
            </div>
          </div>
        </NeonCard>
        ...
        {/* Audio Metrics (4D) */}
        <NeonCard title='Audio Performance (4D)' variant='accent' interactive={false}>
          <div className='space-y-4'>
            <div className='grid grid-cols-2 gap-4 text-sm'>
              <div>
                <span className='text-gray-400'>Audio Latency:</span>
                <span className='ml-2 font-mono text-white'>{metrics.audioLatency.toFixed(1)}ms</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className='ml-2 font-mono text-pink-400'>{metrics.frequencyPeaks.length}</span>
              </div>
            </div>
      ...
      {metrics.optimizationSuggestions.length > 0 && (
        <NeonCard title='Performance-Optimierungen' variant='ghost' interactive={false}>
          <div className='space-y-4'>
            <div className='flex items-center justify-between'>
              <span className='text-sm text-gray-400'>Erkannte Optimierungsmöglichkeiten:</span>
