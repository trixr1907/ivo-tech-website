...
export function CLIInterface() {
  ...
  return (
    <div className='text-center'>
      <h2 className='mb-8 text-3xl font-bold text-white md:text-4xl'>Entwickler-Terminal</h2>
          ...
          <div className='space-y-2 font-mono text-sm'>
            {commandHistory.map((line, index) => (
              <div key={index} className={line.startsWith('$') ? 'text-green-400' : 'text-gray-300'}>
                {line}
                {index === commandHistory.length - 1 && <span className='animate-pulse'>|</span>}
              </div>
            ))}
          </div>
        </div>

        <p className='mt-6 text-gray-400'>Echte Entwicklungsumgebung - Live-Updates und moderne Tools</p>
      </div>
    </div>
  );
}
