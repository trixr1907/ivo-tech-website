...
const cryptoIds = ['bitcoin', 'ethereum', 'binancecoin', 'solana', 'cardano', 'avalanche-2', 'polkadot', 'chainlink'];
...
export function CryptoDashboard() {
  ...
  const fetchCryptoData = async () => {
    try {
      ...
      const selected = response.data.find((crypto: CryptoData) => crypto.id === selectedCrypto);
      if (selected && selected.sparkline_in_7d) {
        const history = selected.sparkline_in_7d.price.map((price: number, index: number) => ({
          timestamp: Date.now() - (selected.sparkline_in_7d!.price.length - index) * 60 * 60 * 1000,
          price: price,
        }));
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setPriceHistory(prev => {
        const newPoint = {
          timestamp: Date.now(),
          price: prev[prev.length - 1]?.price * (0.995 + Math.random() * 0.01) || 45000,
        };
  ...
  return (
    <div className='space-y-6'>
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className='mb-8 text-center'>
        <div className='mb-4 font-mono text-sm text-cyan-400'>$ crypto --live</div>
        <h2 className='text-3xl font-bold text-white md:text-4xl'>
          Live{' '}
          <span className='bg-gradient-to-r from-yellow-400 to-orange-500 bg-clip-text text-transparent'>Crypto</span>{' '}
          Dashboard
        </h2>
        <p className='mt-2 text-gray-400'>
          Echtzeit-Kryptowährungsdaten • Letzte Aktualisierung: {lastUpdate.toLocaleTimeString('de-DE')}
        </p>
        {error && <div className='mt-2 rounded bg-orange-500/10 px-3 py-1 text-sm text-orange-400'>⚠️ {error}</div>}
      </motion.div>

      {/* Crypto Cards Grid */}
      <div data-testid='crypto-grid' className='grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4'>
        {cryptoData.map((crypto, index) => (
          ...
          >
            ...
            <div className='space-y-2'>
              <div>
                <div className='text-2xl font-bold text-white'>{formatPrice(crypto.current_price)}</div>
              ...
              <div className='border-t border-gray-700 pt-2'>
                <div className='flex justify-between text-sm'>
                  ...
                  <span className='text-gray-200'>{formatMarketCap(crypto.market_cap)}</span>
                </div>
                <div className='flex justify-between text-sm'>
                  <span className='text-gray-400'>Volume:</span>
                  <span className='text-gray-200'>{formatMarketCap(crypto.total_volume)}</span>
                </div>
              </div>
            </div>

            {selectedCrypto === crypto.id && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className='absolute right-2 top-2'>
          ...
          >
            <div className='mb-4 flex items-center justify-between'>
              <h3 className='text-xl font-bold text-white'>
                {cryptoData.find(c => c.id === selectedCrypto)?.name || 'Selected'} Price Chart
              </h3>
            ...
            <div className='h-64'>
              <ResponsiveContainer width='100%' height='100%'>
                <AreaChart data={priceHistory}>
                  ...
                  <XAxis
                    dataKey='timestamp'
                    tickFormatter={value => new Date(value).toLocaleTimeString('de-DE', { timeStyle: 'short' })}
                    stroke='#9ca3af'
                  />
                  <YAxis
        ...
        <div className='grid grid-cols-2 gap-4 md:grid-cols-4'>
          ...
          <div className='text-center'>
            <div className='text-2xl font-bold text-blue-400'>
              {formatMarketCap(cryptoData.reduce((sum, c) => sum + c.market_cap, 0))}
            </div>
            <div className='text-sm text-gray-400'>Total Market Cap</div>
          </div>
          <div className='text-center'>
            <div className='text-2xl font-bold text-purple-400'>
              {formatMarketCap(cryptoData.reduce((sum, c) => sum + c.total_volume, 0))}
            </div>
