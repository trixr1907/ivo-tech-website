...
import { AudioReactiveData, SceneContextState, PostProcessingConfig, CyberpunkShaderUniforms } from '../types';
...
class AudioAnalyzer {
  ...
  private getFrequencyRange(data: Float32Array, start: number, end: number): number {
 ...
 */
export function SceneProvider({ children, enableAudio = true, initialPreset = 'city-night' }: SceneProviderProps) {
  ...
  useFrame((state, delta) => {
    ...
    const audioData = enableAudio ? audioAnalyzer.current.getAudioData() : sceneState.audioData;

    // Audio-reaktive Farb-Shifts (reduziert auf Mobil)
    const colorShiftIntensity = isMobile ? 0.3 : 0.5;
    const colorShift = audioData.bass * colorShiftIntensity + Math.sin(time * 0.1) * 0.2;
    const glitchIntensity = audioData.amplitude * (isMobile ? 1.0 : 2.0);

    setSceneState(prev => ({
      ...prev,
      audioData,
      time,
      mouse: mouseRef.current.clone(),
      resolution: new THREE.Vector2(size.width, size.height),
      shaderUniforms: {
        uTime: { value: time },
        uAudioData: { value: audioData },
        uResolution: { value: new THREE.Vector2(size.width, size.height) },
        uMouse: { value: mouseRef.current.clone() },
        uColorShift: { value: colorShift },
        uGlitchIntensity: { value: glitchIntensity },
        uNeonIntensity: { value: 1.0 + audioData.high * (isMobile ? 0.3 : 0.5) },
        uFlowSpeed: { value: 1.0 + audioData.mid * (isMobile ? 1.0 : 2.0) },
      },
    }));
  });
...
export function useSceneContext(): SceneContextState {
  ...
  if (!context) {
    throw new Error('useSceneContext muss innerhalb eines SceneProvider verwendet werden');
  }
  return context;
}
